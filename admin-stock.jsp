        ...
        <div class="w3-w3-container w3-center w3-padding">
            ...
            <%
                ...
                if (stock == null) out.println("GoodsList is missing");
                        ...
                        out.println("<div class=\"scroll-table\"><table class=\"w3-hoverable w3-brown w3-text-white w3-border w3-bordered w3-centered\"><thead><tr><th>Артикул</th><th>Тип товара</th><th>Название товара</th><th>Цена</th><th>Кол-во на складе</th></tr></thead></table>");
                        out.println("<div class=\"scroll-table-body w3-hoverable w3-white w3-text-brown w3-border w3-bordered w3-centered\"><table placeholder=\"Товар не найден\"><tbody placeholder=\"Товар не найден\">");
                        if(stock.size()==0){
                        out.println("<tr><td class=\"w3-centered w3-border-0\">Товар не найден</td></tr>");
                        }
                        else{
                        for (Goods g : stock){
                            out.println("<tr class=\"w3-text-brown\" onclick=\"func(this)\"><td>" + g.getItem().getArticle() + "</td><td>" + g.getItem().getType().getText() + "</td><td>" + g.getItem().getName() + "</td><td>" + g.getItem().getPrice() +(g.getItem().getMeasure()==Measures.gram? " р/100":" р/")+ g.getItem().getMeasure().getText() +"</td><td>" + g.getQuantity() + "</td></tr>");
                        }
                        }
         ...
         <div class="w3-container w3-sand w3-text-brown w3-padding">
            <form method="POST">
            <button type="submit" class="w3-btn w3-right w3-round-large" formaction="/stock?action=addItem" >Добавить товар в ассортимент</button>
            </form>
        </div>

        <form method="POST" id="chooseForm" action="/stock?action=change" >
            <input type="text" class = "w3-left-align w3-text-brown w3-border-0 w3-sand" style="display:none" id="tarticle" name="tarticle" readonly>
        </form>

        <div class="w3-container w3-text-brown w3-padding">
            <form method="POST">
                <button type="submit" class="w3-button w3-right w3-sand w3-text-brown w3-hover-text-grey w3-hover-sand w3-padding-small w3-round-large" style="font-size: 11pt"  formaction="/stock?action=toAccountMenu" >Управление аккаунтами</button>
                <button type="submit" class="w3-button w3-left w3-sand w3-text-brown w3-hover-text-grey w3-hover-sand w3-padding-small w3-round-large" style="font-size: 11pt"  formaction="/stock?action=signout" >В главное меню</button>
            </form>
        </div>
        </div>
    </body>
